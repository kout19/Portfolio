    ...
    <div className="fixed inset-0  z-50 bg-black bg-opacity-50 flex items-center justify-center">
      <div className="bg-white p-6 rounded shadow-md w-full max-w-md relative mt-[100px]">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <button
            type="submit"
            className={`button ${isSignUp ? 'bg-gray-400 text-gray-700 cursor-not-allowed' : 'hover:bg-blue-700'} w-full bg-blue-600 text-white py-2 rounded`}
            disabled={loading} // Disable the button while loading
          >
            {loading ? (
              <ClipLoader color="#fff" loading={loading} size={20} /> // Display the loader when loading
            ) : (
              isSignUp ? 'Sign Up' : 'Sign In' // Show text when not loading
            )}
          </button>

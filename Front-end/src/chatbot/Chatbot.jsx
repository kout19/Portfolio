...
const ChatBot = () => {
  ...
  const [messages, setMessages] = useState([
    { role: 'bot', text: "Hi! I'm the AI assistant for Kefyalew. Ask me about his experience or contact info!" }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/chat?user_message=${encodeURIComponent(input)}`);
      const data = await response.json();
      setMessages(prev => [...prev, { role: 'bot', text: data.reply }]);
    } catch (error) {
      setMessages(prev => [...prev, { role: 'bot', text: "Sorry, my brain is offline. Try again later!" }]);
    } finally {
      setIsTyping(false);
    }
  };
